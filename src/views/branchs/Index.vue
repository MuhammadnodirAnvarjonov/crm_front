<template>
  <div class="p-4 md:p-6 bg-white rounded-3xl h-full shadow-xl border border-gray-100 flex flex-col">
    ...
    <div class="flex-1 min-h-0 overflow-hidden rounded-xl border border-gray-100 bg-gray-50/30">
      <ResponsiveTable :columns="columns" :data="filteredBranchs" @refresh="getModels">
    ...
    <Transition name="modal">
      <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <!-- Modal Content -->
        <div
          class="modal-content relative bg-white rounded-2xl shadow-2xl w-full max-w-md md:max-w-lg flex flex-col max-h-[90vh]">
          <!-- Modal Header -->
    ...
    <Transition name="modal">
      <div v-if="showViewModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="modal-content relative bg-white rounded-2xl shadow-2xl w-full max-w-md md:max-w-lg p-6">
          ...
          <View :branch="selectedBranch" @close="closeView" />
        </div>
      </div>
    </Transition>
    ...
    <ConfirmModal v-if="showConfirmModal" :show="showConfirmModal" :title="$t('delete')"
      :message="`${itemToDelete?.name} filialini o‘chirishni tasdiqlaysizmi?`" confirm-text="Ha, o‘chirish"
...
const getModels = async () => {
  branchs.value = await branchservice.all();
  searchedOnce.value = false;
};

const filteredBranchs = computed(() => {
  const q = searchQuery.value.trim().toLowerCase();
  if (!q) return branchs.value;
  return branchs.value.filter((b) => b.name?.toLowerCase().includes(q));
});
...
watch(searchQuery, (val) => {
  ...
  debounceTimer = setTimeout(async () => {
    ...
    const local = branchs.value.filter((b) =>
      b.name?.toLowerCase().includes(val.trim().toLowerCase())
    );
    if (local.length === 0 && !searchedOnce.value) {
      try {
        const result = await branchservice.all({ text: val });
        if (result?.length) {
          branchs.value = [...branchs.value, ...result];
        }
...
const viewItem = (item) => { selectedBranch.value = item; showViewModal.value = true; };
const closeView = () => { showViewModal.value = false; selectedBranch.value = null; };

// DELETE FUNCTIONALITY (ConfirmModal orqali)
...
const handleConfirmDelete = async () => {
  ...
  try {
    await branchservice.delete(itemToDelete.value.id);
