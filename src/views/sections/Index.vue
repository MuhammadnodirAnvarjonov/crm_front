<template>
  <div class="p-4 md:p-6 bg-white rounded-3xl h-full shadow-xl border border-gray-100 flex flex-col">
    ...
    <div class="flex-1 min-h-0 overflow-hidden rounded-xl border border-gray-100 bg-gray-50/30">
      <!-- @row-click hodisasini saqlab qoldik -->
      <ResponsiveTable :columns="columns" :data="filteredModels" @refresh="getModels" @row-click="selectRow">
    ...
    <ConfirmModal v-if="showConfirmModal" :show="showConfirmModal" :title="$t('delete')"
      :message="`${itemToDelete?.name} bo‘limini o‘chirishni tasdiqlaysizmi?`" confirm-text="Ha, o‘chirish"
...
const columns = [
  ...
  { key: "audio", label: t('audio'), class: "text-gray-500" },
...
// Row click (parentga emit qilish)
const selectRow = (row) => emit("selectRow", row);
