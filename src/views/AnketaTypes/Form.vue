<template>
  <form @submit.prevent="save" class="flex flex-col">
    ...
    <div class="px-1 py-4 border-t border-gray-100 dark:border-slate-700 mt-4 flex justify-end gap-3 sticky bottom-0 bg-white dark:bg-slate-800">
      ...
      <BaseButton type="submit" status="primary" :loading="saving">{{ $t('save') }}</BaseButton>
    </div>
  </form>
</template>
...
const save = async () => {
  ...
  try {
    ...
    if (props.editData?.id) {
      await anketaTypesService.update(props.editData.id, payload)
    } else {
      await anketaTypesService.create(payload)
    }
    emit('saved')
    emit('close')
  } catch (e) {
    console.error('Anketa type save error', e)
  } finally {
    saving.value = false
  }
}
