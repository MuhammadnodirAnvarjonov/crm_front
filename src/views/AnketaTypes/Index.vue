...
const getItems = async () => {
  items.value = await anketaTypesService.all()
...
watch(searchQuery, (val) => {
  ...
  debounceTimer = setTimeout(async () => {
    ...
    if (local.length === 0 && !searchedOnce.value) {
      try {
        const result = await anketaTypesService.all({ text: val })
        if (result?.length) items.value = [...items.value, ...result]
...
const handleConfirmDelete = async () => {
  ...
  try {
    await anketaTypesService.delete(itemToDelete.value.id)
